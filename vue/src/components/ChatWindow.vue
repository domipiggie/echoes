  ...
  <div class="chat-window container-fluid p-0" :class="currentTheme">
    <div class="chat-header row m-0 align-items-center">
      <div class="col d-flex align-items-center">
        <div class="user-info">
          <button class="back-button btn" @click="$emit('go-back')" v-if="isMobile">
    ...
    <div class="messages-container" ref="messagesContainer">
      ...
      <template v-if="currentTheme === 'discord'">
        <div v-for="(message, index) in messages" :key="message.id" class="message discord-style" :class="[
            ...
            <div class="discord-message-content">
              ...
              <div class="discord-message-wrapper">
                <div class="message-bubble discord-bubble">
                  <!-- Kép és GIF kezelés -->
                  <img v-if="message.type === 'gif' || message.type === 'image'" :src="message.text" class="message-gif"
                  ...
                  <!-- Discord template videó kezelés -->
                  <div v-else-if="message.type === 'video'" class="video-container">
                    <video :src="message.text" class="message-video" controls @click.stop ref="videoRef"
                    ...
                    <div class="video-options">
                      ...
                      <div v-if="showVideoOptions[message.id]" class="video-options-menu">
                        <div class="video-option-item"
                          @click.stop="toggleFullscreen($event.target.closest('.video-container').querySelector('video'))">
                          Teljes képernyő
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                            <path
                        ...
                        <!-- Add mute toggle option -->
                        <div class="video-option-item"
                          @click.stop="toggleMute($event.target.closest('.video-container').querySelector('video'), message.id)">
                          {{ videoStates[message.id]?.muted ? 'Unmute' : 'Mute' }}
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2">
                  ...
                  <!-- Regular template videó kezelés -->
                  <div v-else-if="message.type === 'video'" class="video-container">
                    <video :src="message.text" class="message-video" controls @click.stop ref="videoRef"
                      preload="metadata"></video>
                    <div class="video-options">
                  ...
                  <!-- Szöveges üzenetek -->
                  <span v-if="message.type !== 'audio' && message.type !== 'file'">{{ message.text }}</span>
    ...
    </div> <!-- Fixed: Added closing div for chat-window -->
</template>
