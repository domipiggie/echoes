...
import DiscordMessage from '../../components/chat/messages/DiscordMessage.vue';
...
describe('DiscordMessage', () => {
  ...
  it('displays user avatar for first consecutive message', () => {
    ...
    expect(wrapper.find('.discord-avatar').exists()).toBe(true);
  });
  ...
  it('hides avatar for consecutive messages from same user', () => {
    ...
    expect(wrapper.find('.discord-avatar-placeholder').exists()).toBe(true);
  });
  ...
  it('shows fallback text for missing replied messages', async () => {
      ...
      expect(wrapper.find('.discord-reply-indicator .reply-text').text()).toContain('Üzenet betöltése sikertelen');
  });
